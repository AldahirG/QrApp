    ...
    try {
      ...
      for (const { url, setter } of endpoints) {
        const response = await fetch(
          `${BASE_URL}/api/registros${url}?conferencista=${encodeURIComponent(conferencista)}`
        );
...
const downloadExcel = async () => {
  ...
  const url = `${BASE_URL}/api/registros/export/excel?conferencista=${encodeURIComponent(eventoSeleccionado)}`;
  Linking.openURL(url);
};
